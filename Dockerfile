...
RUN echo LC_ALL=en_US.UTF-8 >>/etc/locale.conf && \
		...
		\
		pacman -Syu --noconfirm base python-pipenv cmake ninja git gcc which openssh lld ccache bison flex rsync make wget awk gperftools crypto++ diffutils moreutils time patch file fish zsh && \
		\
