...
class _Config:
    ...
    def add_argument(self, argparser: ArgumentParser, help: str = f'The chosen configuration'):
        ...
        return argparser.add_argument(name, metavar=NAME, nargs="?", choices=self.choices, help=f'{help} (choices: {", ".join(self.choices)}) (env: WS_{NAME})')

    def add_kwargument(self, argparser: ArgumentParser, help: str = f'The chosen configuration'):
        NAME = self.name.upper().replace("-", "_")
        return argparser.add_argument("-c", "--config", metavar=NAME, choices=self.choices, help=f'{help} (choices: {", ".join(self.choices)}) (env: WS_{NAME})')
...
class _Configs:
    ...
    def add_argument(self, argparser: ArgumentParser, help: str = f'The set of chosen configurations'):
        ...
        return argparser.add_argument(name, metavar=NAME, nargs="*", default=[], help=f'{help} (choices: {", ".join(self.choices)}) (env: WS_{NAME})')
    ...
    def _understand_value(self, value: List[str]) -> List[str]:
        ...
        for config in value_set:
            ...
            if config not in self.choices:
                raise Exception(f'"{config}" is not a valid configuration (encountered while parsing the "{self.name}" setting)')
...
class _DefaultLinker:
    ...
    def add_kwargument(self, argparser: ArgumentParser, help: str = f'The default linker'):
        NAME = self.name.upper().replace("-", "_")
        return argparser.add_argument('--default-linker', choices=self.choices, metavar=NAME, help=f'{help} (choices: {", ".join(self.choices)}) (env: WS_{NAME})')
    ...
    def _understand_value(self, value: List[str]) -> List[str]:
        ...
        for recipe in value_set:
            ...
            if recipe not in self.choices:
                raise Exception(f'"{recipe}" is not a valid recipe (encountered while parsing the "{self.name}" setting)')
        return list(value_set)
...
class _ReferenceRepositories:
    ...
    def add_kwargument(self, argparser: ArgumentParser, help: str = "The location of the reference repositories") -> None:
        NAME = self.name.upper().replace("-", "_")
...
class _XGitClone:
    ...
    def add_kwargument(self, argparser: ArgumentParser, help: str = "Additional arguments to pass to the underlying git clone call") -> None:
        NAME = self.name.upper().replace("-", "_")
